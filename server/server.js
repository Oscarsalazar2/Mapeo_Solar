...
const bd = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgres://postgres:postgres@localhost:5432/mapeo_solar'
});
...
servidor.get('/api/reports', async (req, _reply) => {
  ...
  const vista =
    rango === 'day'  ? 'lecturas_dia' :
    rango === 'week' ? 'lecturas_semana' :
                       'lecturas_mes';
...
servidor.post('/api/lecturas', async (req, reply) => {
  ...
  if (!sensor_id || lux === undefined || lux === null) {
    return reply.code(400).send({ ok: false, error: 'sensor_id y lux son requeridos' });
  }
